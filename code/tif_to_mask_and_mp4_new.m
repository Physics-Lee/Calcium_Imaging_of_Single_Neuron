function tif_to_mask_and_mp4_new(folder_path,sensitivity_threshold,is_test)
...
if is_test
    start_frame = 1;
    end_frame = 300;
    video_name_str = sprintf('output_video_sense_%.4f_from_%d_to_%d_new.mp4',sensitivity_threshold,start_frame,end_frame);
else
    start_frame = 1;
    end_frame = length(files);
    video_name_str = sprintf('output_video_sense_%.4f_new.mp4',sensitivity_threshold);
...
for i = start_frame:end_frame
    ...
    % Binarization Method 2    
    c_filtered=imfilter(gray_frame,h);
